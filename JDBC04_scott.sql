...
-- 2. 리스트 추력 쿼리문 구성(총점, 평균, 석차 포함)
SELECT SID, NAME, ENG, MAT
     , (KOR+ENG+MAT) AS TOT
...
ORDER BY SID ASC;
--> 한 줄 구성
SELECT SID, NAME, ENG, MAT, (KOR+ENG+MAT) AS TOT, (KOR+ENG+MAT)/3 AS AVG, RANK() OVER(ORDER BY (KOR+ENG+MAT) DESC) AS RANK FROM TBL_SCORE ORDER BY SID ASC
;
--==>> 1	이다영	80	70	240	80	1
